...
import { Send, Trash2, Edit2, Shield, User as UserIcon, Lock, Clock, Coins, Crown, MessageCircle, AlertTriangle, Ban, ArrowLeft, Globe, ToggleRight, ToggleLeft, Palette, Sparkles, X, Reply } from 'lucide-react';
import { CreditConfirmationModal } from './CreditConfirmationModal';
import { CustomConfirm } from './CustomDialogs';
import { rtdb } from '../firebase';
import { ref, onValue, push, set, update, remove, get, query, limitToLast } from "firebase/database";
...
export const UniversalChat: React.FC<Props> = ({ currentUser, onUserUpdate, isAdminView = false, settings }) => {
  ...
  useEffect(() => {
      ...
      if (isAdminView) {
          if (activeTab === 'UNIVERSAL') {
              ...
              // OPTIMIZATION: Limit to last 50 active students to prevent admin lag
              const chatsRef = query(ref(rtdb, 'chats'), limitToLast(50));
      ...
      if (chatPath) {
          // OPTIMIZATION: Limit to last 100 messages to prevent 10s delay
          const chatRef = query(ref(rtdb, chatPath), limitToLast(100));
  ...
  useEffect(() => {
      if (isAdminView && activeTab === 'PRIVATE' && selectedStudentId) {
          // OPTIMIZATION: Limit to last 100
          const chatRef = query(ref(rtdb, `chats/${selectedStudentId}/messages`), limitToLast(100));
          const unsub = onValue(chatRef, (snapshot) => {
              const data = snapshot.val();
              if (data) {
                  const msgList: ChatMessage[] = Object.values(data);
  ...
  return (
    <div className={`flex flex-col h-[80vh] bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200 ${isAdminView ? '' : 'max-w-4xl mx-auto'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
            {messages.map((msg) => {
                ...
                if (isMe) {
                    ...
                    // ALL OTHER USERS (Premium or Free) - Simple Style
                    bubbleClass = 'bg-white text-slate-800 border border-slate-200 rounded-tl-none';
                }
                ...
                return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} ${animationClass}`}>
                         <div className={`max-w-[85%] rounded-2xl p-3 shadow-sm relative group transition-all duration-300 ${bubbleClass} ${glowClass}`}>
                             ...
                             {/* Reply Context */}
                             {msg.replyTo && (
                                 <div className={`text-[10px] mb-1 p-1 rounded border-l-2 opacity-80 ${isMe ? 'bg-blue-700/30 border-blue-200 text-blue-100' : 'bg-slate-100 border-slate-300 text-slate-500'}`}>
                             ...
                             <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-black/10 backdrop-blur-md rounded p-0.5">
                                 <button onClick={() => setReplyingTo(msg)} className="p-1 text-slate-200 hover:text-white" title="Reply">
                                     <Reply size={12} />
                                 </button>
                                 {(currentUser.role === 'ADMIN') && (
                                     <button onClick={() => requestDelete(msg.id)} className="p-1 text-slate-200 hover:text-red-300" title="Delete">
                                         <Trash2 size={12} />
                                     </button>
                                 )}
                             </div>
        ...
        <div className="p-3 bg-white border-t border-slate-200">
            {replyingTo && (
                <div className="flex justify-between items-center bg-blue-50 p-2 rounded-lg mb-2 border-l-4 border-blue-500 animate-in slide-in-from-bottom-2">
                    <div className="text-xs text-blue-700">
                        <span className="font-bold">Replying to {replyingTo.userName}</span>
                        <p className="truncate opacity-70 w-64">{replyingTo.text}</p>
                    </div>
